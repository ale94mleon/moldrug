...
def CostMultiReceptors(Individual:utils.Individual, wd:str = '.vina_jobs', receptor_path:list = None, boxcenter:list = None, vina_score_types:list = None, boxsize:list =None, exhaustiveness:int = 8, ncores:int = 1,  num_modes:int = 1):
    ...
    for i in range(len(receptor_path)):
        cmd = f"{utils.vina_executable} --receptor {receptor_path[i]} --ligand {os.path.join(wd, f'{Individual.idx}_{i}.pdbqt')} "\
